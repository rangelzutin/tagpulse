...
import { createProductionProductSyncRunner } from "../src/modules/products/production-product-sync.js";
...
function harness(connection: object | null, withToken = true) {
  ...
  const prisma = {
    ...
    productSyncRun: { findFirst: vi.fn().mockResolvedValue(null) },
...
describe("production product sync preflight", () => {
  ...
  it("rejects an inactive connection before fetch", async () => {
    ...
    await expect(h.runner.preflight(connectionId)).rejects.toMatchObject({
      category: "PRODUCT_SYNC_CONNECTION_INACTIVE",
    });
    expect(h.fetch).not.toHaveBeenCalled();
  });
  ...
  it("rejects test database targets before querying or fetching", async () => {
    const h = harness(activeConnection);
    const unsafe = createProductionProductSyncRunner({
      prisma: h.prisma,
    ...
    await expect(unsafe.preflight(connectionId)).rejects.toMatchObject({
      category: "PRODUCT_SYNC_UNSAFE_DATABASE",
    });
    expect(h.fetch).not.toHaveBeenCalled();
  });
});
